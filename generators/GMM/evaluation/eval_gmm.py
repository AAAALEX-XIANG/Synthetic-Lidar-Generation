...
def main(eval_config, args):
    ...
    # Load Gaussian Mixture Model and Decoder
    gm = joblib.load(args.model_GMM)
    G.load_state_dict(torch.load(args.model_g))

    train_start_time = time.time()
    print("Start Evaluation !")
    with torch.no_grad():
        samples, _ = gm.sample(num_samples)
        X_g = G(torch.tensor(samples.tolist()).to(torch.device('cuda')))
    ...
    for k, v in results.items():
        print('%s: %.12f' % (k, v))
    print('Evaluating GMM finished, took {:.2f}s'.format(time.time() - train_start_time))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_GMM', type=str, required=True, help='GMM path')
