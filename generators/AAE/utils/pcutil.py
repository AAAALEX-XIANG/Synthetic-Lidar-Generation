...
def plot_3d_point_cloud(x, y, z, reconstruction, show=True, show_axis=True, in_u_sphere=False,
    ...
    print("Types x y z:", type(x), type(y), type(z))

    if(not reconstruction):
        a = x.cpu().detach().numpy()
        b = y.cpu().detach().numpy()
        c = z.cpu().detach().numpy()
        sc = ax.scatter(a, b, c, marker=marker, s=s, alpha=alpha, *args, **kwargs)
    else:
        a = x
        b = y
        c = z
    ...
    if in_u_sphere:
        ...
        miv = 0.7 * np.min([np.min(a), np.min(b), np.min(c)])
        mav = 0.7 * np.max([np.max(a), np.max(b), np.max(c)])
        ax.set_xlim(miv, mav)
        ax.set_ylim(miv, mav)
        ax.set_zlim(miv, mav)
        plt.tight_layout()
