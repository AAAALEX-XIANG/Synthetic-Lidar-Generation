...
def main(eval_config, args):
    ...
    De.load_state_dict(torch.load(eval_config["decoder"]))
    ...
    results['jsd'] = jsd
    print('Coverage  | CD %.6f' % (results['lgan_cov-CD'], ))
    print('MinMatDis | CD %.6f' % (results['lgan_mmd-CD'], ))
    print('NN-Accur | CD %.6f' % (results['1-NN-CD-acc'], ))
    print('JsnShnDis | %.6f ' % (results['jsd']))
    print('Evaluating GAN finished, took {:.2f}s'.format(time.time() - train_start_time))
